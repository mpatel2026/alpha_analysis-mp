...
#SBATCH --array=54
#SBATCH --output=logs-CPU/ascot_%a.out
#SBATCH --error=logs-CPU/ascot_%a.err
...
INPUT_FILE=$(sed -n "${SLURM_ARRAY_TASK_ID}p" sim_list.txt)
...
srun -n $SLURM_NNODES -c 256 --cpu_bind=cores "${HOME_DIR}ascot5/ascot5_develop_mp_hpc/build/ascot5_main" --in="${SCRATCH_DIR}$INPUT_FILE" --d="$RUN_MESSAGE"
